...
So the entire effect comes from a minority of catastrophic episodes in which the arm becomes
entangled with the obstacle and accumulates thousands of contact frames. Proximity does not make
routine avoidance smoother. It makes the disaster case rarer.
...
This also explains why task success barely moves: the episodes proximity rescues were largely
ones already failing the manipulation, so removing the contact does not convert them into
successes.
