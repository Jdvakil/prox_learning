...
**T-v6** (2026-09-23, V10.10 two-object, 200 demos, exo+wrist, **chunk 100**, `eval_act_place.py --env v6`, seeds 2026–2075, cycle_24, consecutive history, n=50): ACT 20/5/28, PACT-raw 21/1/29, **PACT-readout 21/1/38** (place / bar / collision-free); readout vs ACT contact-free McNemar 1 vs 11, p = 0.006; placement flat — `eval_output/pact_pick_n_place_v2_v6_{ACT,PACT_RAW,PACT_READOUT}_s0_bs8_cs100_lr1e-5_e2000/`, PAPER.md §3.10. **T-v12** (2026-09-23, kitchen bench, chunk 50, same protocol): ACT 30/7/34, **PACT-raw 26/0/44**, PACT-readout 26/3/36; raw vs ACT bar 0 vs 7 (p = 0.016), coll-free 12 vs 2 (p = 0.013); readout vs ACT n.s.; PAPER.md §3.11. **T-1011c** (demo-side cups): ACT 12/10/18, raw 12/0/25, readout 11/1/27 (bar ACT vs readout 9 vs 0, p = 0.004). **T-1010**: 25/2/32, 23/1/35, 31/1/34 (n.s.). **T-107h**: 24/5/20, 26/3/29, 29/1/26 (trends). PAPER.md §3.12–3.14; all in `README_RESULTS.md`.
...
**Camera blank (skin-only test).** `--blank_cameras all` (or camera names) on `eval_act.py`, `eval_act_v1011d.py` and `eval_act_place.py` feeds the policy an all-black RGB frame (uint8 0, then /255, before ACT's ImageNet normalisation) for those cameras at every query; skin and qpos unchanged, env still renders the cameras (videos / first frames / contact audit unchanged). Protocol field (only when set): resume refuses a blank mismatch both ways. Helper: [`scripts/pact_eval_camera_blank.py`](scripts/pact_eval_camera_blank.py). Shells: `BLANK_CAMERAS=all` on every `eval_<env>.sh` with a keep knob (v6, v12, v1010, v1011c, v1011d_tstrain, v107_spaced hub / batman) and `eval_v1_hallway.sh`; output dir gains `_blind` (all) or `_no<cam>` (`exo_camera_1` → `_noexo`). ACT with every camera blank sees qpos only — the control.
