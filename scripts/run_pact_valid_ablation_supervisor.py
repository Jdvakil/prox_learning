#!/usr/bin/env python3
"""Durable eight-worker supervisor for the 40-row valid ablation."""
...
EVALUATOR = ROOT / "submodules/act/eval_pact_valid_ablation_row.py"
EXPECTED_ROWS = 40


class ValidAblationSupervisor(screen.ScreenSupervisor):
    def __init__(
        self,
        *,
        schedule_path: Path,
        ...
        if screen.canonical_hash(payload) != observed:
            raise RuntimeError("valid-ablation schedule self-hash mismatch")
        if (
            self.schedule.get("schema_version")
            != "pact_valid_ablation_schedule_v1"
            or self.schedule.get("workers") != screen.WORKERS
            or self.schedule.get("rollouts") != EXPECTED_ROWS
            or len(self.schedule.get("rows", [])) != EXPECTED_ROWS
        ):
            raise RuntimeError("valid-ablation schedule design mismatch")
        ...
        if (
            ...
            != "pact_valid_ablation_dispatch_v1"
        ):
            raise RuntimeError("wrong valid-ablation dispatch contract")
        for label, record in self.contract["frozen_inputs"][
            "runtime"
        ].items():
            if screen.base.sha256_file(Path(record["path"])) != record[
                "sha256"
            ]:
                raise RuntimeError(
                    f"frozen valid-ablation runtime changed: {label}"
                )
    ...
    def _command(
        ...
        *,
        ...
        if recovery_event is not None:
            command.extend(
                ["--inflight-recovery-event", str(recovery_event)]
            )
        return command
    ...
    def run(self) -> int:
        ...
        pid_record = {
            "schema_version": "pact_valid_ablation_supervisor_pid_v1",
            "pid": os.getpid(),
            "identity": identity,
            "mode": self.mode,
        ...
        target = 1 if self.mode == "smoke" else EXPECTED_ROWS
        ...
        if self.mode == "smoke":
            self._write_smoke()
        summary = {
            "schema_version": "pact_valid_ablation_execution_v1",
            "schedule_sha256": self.schedule["schedule_sha256"],
        ...
        screen.base.write_json_atomic(
            self.output_root / f"{self.mode}_execution_summary.json",
            summary,
        )
...
def main() -> int:
    ...
    return ValidAblationSupervisor(
