#!/usr/bin/env python3
"""Run smoke or full chunk-100 place rollouts with bounded workers."""
...
EVALUATORS = {
    "ACT": ROOT / "submodules" / "act" / "eval_pact_place_chunk100_row.py",
    "PACT": ROOT / "submodules" / "act" / "eval_pact_place_chunk100_row.py",
    "PACT_PERMUTED": ROOT / "submodules" / "act" / "eval_pact_place_permuted_row.py",
}
...
ARMS = {
    "ACT": {
        "checkpoint_dir": Path("/root/pact_place_152_pact_vs_act_chunk100_seed3101/act_seed3101"),
        "checkpoint_sha256": None,
    },
    "PACT": {
        "checkpoint_dir": Path("/root/pact_place_152_pact_vs_act_chunk100_seed3101/pact_seed3101"),
        "checkpoint_sha256": None,
    },
    "PACT_PERMUTED": {
        "checkpoint_dir": Path("/root/pact_place_152_pact_vs_act_chunk100_seed3101/pact_seed3101"),
...
TOKEN_PLAN_SHA256: str | None = None
...
def schedule_row(row: dict[str, Any], arm: str) -> dict[str, Any]:
    rollout_id = f"chunk100_{arm.lower()}_{int(row['role_index']):03d}_{row['episode_id'][:16]}"
    payload = {
        "arm": arm,
        ...
        "num_queries": 100,
        "token_plan_sha256": TOKEN_PLAN_SHA256 if arm == "PACT_PERMUTED" else None,
    }
...
def command_for(
    schedule: dict[str, Any], manifest_path: Path, token_plan_path: Path, row_dir: Path
) -> list[str]:
    arm = schedule["arm"]
    ...
    if arm == "PACT_PERMUTED":
        command.extend(
            [
                "--token-plan-manifest",
                str(token_plan_path.resolve()),
                "--token-plan-row",
                str(schedule["role_index"]),
            ]
        )
    return command
...
def validate_result(path: Path, schedule: dict[str, Any]) -> dict[str, Any]:
    ...
    if info.get("num_queries") != 100:
        raise RuntimeError(f"{path}: checkpoint was not loaded at num_queries=100")
    if info.get("contact_audit_class") != "PactPlaceContactAudit":
        raise RuntimeError(f"{path}: place audit was not attached")
    if schedule["arm"] in ("PACT", "PACT_PERMUTED"):
    ...
    if schedule["arm"] == "PACT_PERMUTED":
        if info.get("live_proximity_aligned_with_action") is not False:
            raise RuntimeError(f"{path}: live proximity alignment was not destroyed")
        if info.get("token_plan_sha256") != schedule["token_plan_sha256"]:
            raise RuntimeError(f"{path}: token-plan identity differs")
        if info.get("surface_encoder_self_verified") is not True:
            raise RuntimeError(f"{path}: encoder was not independently verified")
        if info.get("surface_encoder_verified_sha256") != ENCODER_SHA256:
            raise RuntimeError(f"{path}: self-verified encoder hash differs")
    return result


def run_one(
    row: dict[str, Any], arm: str, manifest_path: Path, token_plan_path: Path, output_root: Path
) -> dict[str, Any]:
    ...
    command = command_for(schedule, manifest_path, token_plan_path, row_dir)
    started = utc_now()
    ...
    driver = {
        "schema_version": "pact_place_eval_chunk100_driver_v1",
        **schedule,
        "command": command,
        "started_utc": started,
...
def main() -> int:
    global TOKEN_PLAN_SHA256
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--manifest", required=True, type=Path)
    parser.add_argument("--output-root", required=True, type=Path)
    parser.add_argument("--token-plan-manifest", required=True, type=Path)
    ...
    if args.mode == "smoke" and tuple(args.arms) != tuple(ARMS):
        raise SystemExit("smoke requires ACT, PACT, and PACT_PERMUTED")
    checkpoint_hashes = {}
    ...
    token_plan = json.loads(args.token_plan_manifest.read_text())
    token_payload = dict(token_plan)
    token_hash = token_payload.pop("token_plan_sha256", None)
    if token_hash != canonical_hash(token_payload):
        raise SystemExit("token-plan self-hash mismatch")
    TOKEN_PLAN_SHA256 = token_hash
    if token_plan.get("source_train_episode_count") != 122:
        raise SystemExit("token plan did not use the 122-episode place train split")
    ...
    summary = {
        "schema_version": "pact_place_eval_chunk100_launcher_summary_v1",
        "mode": args.mode,
        "started_utc": started,
        "finished_utc": utc_now(),
