#!/usr/bin/env python3
"""v12 copy of the expert-screen helpers (PactPlaceCorridorPolicy bind)."""
...
def _make_config(
    ...
    *,
    ...
    config.task_horizon = int(TASK_HORIZON)
    ...
    scene = Path(scene_xml) if scene_xml is not None else (
        ROOT / "custom_scenes" / "pact_place_corridor_v12.xml"
    )
    if sampler_class not in (None, "PactPlaceCorridorV1010FourObjectSampler"):
        raise ValueError(f"v12 binds only PactPlaceCorridorV1010FourObjectSampler, got {sampler_class!r}")
    config.task_sampler_config.task_sampler_class = PactPlaceCorridorV1010FourObjectSampler
