#!/usr/bin/env python3
"""Freeze models, runtime, analysis, and durable geometry dispatch."""
...
from pact_geometry_generalization_contract import sha256_file, sha256_payload
...
def main() -> int:
    ...
    manifest_self_sha = validate_self_hash(manifest, "manifest_sha256", "manifest")
    ...
    if (
        schedule.get("schema_version") != "pact_geometry_generalization_schedule_v1"
        or schedule.get("workers") != WORKERS
        or schedule.get("manifest_sha256") != manifest_self_sha
        ...
        raise ValueError("geometry schedule bindings changed")
    ...
    if output_root.exists() and any(output_root.iterdir()):
        raise ValueError("geometry evaluation output root is not empty")
    smoke = schedule["rows"][0]
    if smoke["condition_id"] != "C0" or smoke["arm"] != "PACT_PERMUTED" or smoke["checkpoint_seed"] != 3101:
        raise ValueError("smoke must exercise C0 PACT_PERMUTED seed 3101")
    runtime_paths = {
        "geometry_contract": ROOT / "scripts/pact_geometry_generalization_contract.py",
        "geometry_schedule_builder": ROOT / "scripts/build_pact_geometry_schedule.py",
        "geometry_dispatch_builder": Path(__file__),
        "geometry_supervisor": ROOT / "scripts/run_pact_geometry_supervisor.py",
        "geometry_launcher": ROOT / "scripts/launch_pact_geometry_detached.py",
        "geometry_full_launcher": ROOT / "scripts/launch_pact_geometry_full.py",
        "geometry_detachment_proof": ROOT / "scripts/prove_pact_geometry_detachment.py",
        "detachment_proof_dependency": ROOT / "scripts/prove_pact_frontend_screen_detachment.py",
        "geometry_compactor": ROOT / "scripts/compact_pact_geometry_storage.py",
        "geometry_throughput": ROOT / "scripts/measure_pact_geometry_throughput.py",
        "geometry_evaluator": ROOT / "submodules/act/eval_pact_geometry_generalization_row.py",
        "contact_evaluator_dependency": ROOT / "submodules/act/eval_pact_contact_endpoint_row.py",
        ...
        "sampler_and_expert": ROOT / "submodules/molmospaces/molmo_spaces/tasks/enclosure_reach.py",
    ...
    document: dict[str, Any] = {
        "schema_version": "pact_geometry_generalization_dispatch_v1",
        "scientific_schedule": {
            "path": str(args.schedule.resolve()),
            "file_sha256": sha256_file(args.schedule),
            "schedule_sha256": schedule_sha,
            "rows": schedule["rollouts"],
            "workers": WORKERS,
            "rows_changed": 0,
        ...
        "frozen_contact_endpoint_hashes": {str(path): digest for path, digest in FROZEN_CONTACT_HASHES.items()},
        "frozen_inputs": {
            "manifest": {"path": str(args.manifest.resolve()), "sha256": sha256_file(args.manifest)},
            "expert_screen": {"path": str(args.expert_screen.resolve()), "sha256": sha256_file(args.expert_screen)},
