#!/usr/bin/env python3
"""Replay-render the 24 attempt-5 place-corridor episodes from recorded qpos.
...
SCREEN_ROOT = ROOT / "diagnostics_output/pact_place_corridor_v5"
DEFAULT_OUTPUT = ROOT / "diagnostics_output/pact_place_corridor_v5_videos"
V3_CONFIG_SHA256 = (
...
FAIL_CLIP_STEMS = {
    3: "row03_FAIL_outbound_approach_ik_cascade",
    10: "row10_FAIL_lift_cup_dropped",
}
...
CRIB_FAILURES = (
    {
        ...
        "phase": "lift",
        "recorded": (
            "never reached 1 cm (cup max z 0.8004 vs 0.7937 start); cup ended "
            "z 0.7527, BELOW its start; empty_gripper branch fired"
        ),
        "watch_for": (
            "Grasp closes but the cup is knocked down rather than lifted. "
            "Does it get pushed off the shelf?"
        ),
    },
    {
        "row": 3,
        ...
        "recorded": (
            "lifted 1 cm and carried, then 8 sequential IK failures; stalled at "
            "y 0.1067 reaching for y 0.1314 (2.5 cm short laterally)"
        ),
        "watch_for": (
            "The genuine reach limit. The arm freezes mid-bow with the cup still "
            "held and nothing blocking it."
        ),
    },
)
...
def _assert_receptacle_placed(task) -> None:
    """The static tray is not in qpos, so object/TCP residuals cannot catch a
    wrong-scene render. Check the receptacle body is where the config puts it."""
    import numpy as np
...
def _prepare_task(row: dict[str, Any], selected_seed: dict[str, int]):
    ...
    try:
        ...
        if scene_xml.name != "pact_place_corridor_v2.xml":
            raise RuntimeError(f"v5 must render the relocated-tray scene, got {scene_xml.name}")
        ...
        task.reset()
        _assert_receptacle_placed(task)
...
def render_row(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if result.get("config_sha256") != V3_CONFIG_SHA256:
        raise RuntimeError(f"row {role_index} is not the frozen v5 screen")
    payload = dict(result)
