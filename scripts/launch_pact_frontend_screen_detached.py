#!/usr/bin/env python3
"""Launch the screen supervisor with setsid/nohup and detached stdin."""
...
def main() -> int:
    ...
    parent_log = (
        output_root / f"{args.mode}_supervisor.log"
    )
    ...
    environment["MLSPACES_ASSETS_DIR"] = (
        "/root/prox_learning_pact_remediation/assets"
    )
    environment["PYTHONPATH"] = (
        f"{ROOT / 'submodules/molmospaces'}:"
        f"{ROOT / 'submodules/act'}:{ROOT / 'scripts'}"
    )
    ...
    receipt = {
        "schema_version": (
            "pact_frontend_screen_launcher_receipt_v1"
        ),
    ...
    receipt["launcher_receipt_sha256"] = canonical_hash(
        receipt
    )
    write_json_atomic(
        output_root / f"{args.mode}_launcher_receipt.json",
        receipt,
    )
    print(process.pid)
    return 0
