#!/usr/bin/env python3
"""Launch detached blind-RGB evaluation, compaction, and throughput monitor."""
...
def main() -> int:
    ...
    if contract.get("schema_version") != "pact_blind_rgb_dispatch_v1":
        raise ValueError("wrong blind-RGB dispatch contract")
    ...
    if (
        ...
        raise ValueError("blind-RGB intervention preflight is invalid")
    if (output_root / "full_launcher_receipt.json").exists():
        raise ValueError("full blind-RGB dispatch already launched")
    ...
    completed = subprocess.run(
        [
            str(PYTHON),
            str(ROOT / "scripts/launch_pact_blind_rgb_detached.py"),
            "--schedule",
    ...
    receipt = {
        "schema_version": "pact_blind_rgb_full_launcher_receipt_v1",
    ...
    receipt.update({"compactor_pid": compactor_pid, "throughput_monitor_pid": throughput_pid})
