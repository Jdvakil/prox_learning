#!/usr/bin/env python3
"""Launch detached attempt-2 evaluation, compaction, and throughput monitor."""
...
def main() -> int:
    ...
    if contract.get("schema_version") != "pact_geometry_generalization_v2_dispatch":
        raise ValueError("wrong attempt-2 geometry dispatch contract")
    ...
    if (output_root / "full_launcher_receipt.json").exists():
        raise ValueError("full attempt-2 geometry dispatch already launched")
    ...
    completed = subprocess.run(
        [
            str(PYTHON),
            str(ROOT / "scripts/launch_pact_geometry_v2_detached.py"),
            "--schedule",
    ...
    receipt = {
        "schema_version": "pact_geometry_generalization_v2_full_launcher_receipt",
