"""Frozen contract for the held-out chunk-1 place rollout evaluation."""
...
SCHEMA_VERSION = "pact_place_eval_chunk1_manifest_v1"
...
def retry_seed(row: dict[str, Any], retry_index: int) -> dict[str, int]:
    entropy = [int(row["master_seed"]), int(row["candidate_index"]), RETRY_STREAM_ID, int(retry_index)]
    low, high = (
        int(value)
        for value in np.random.SeedSequence(entropy).generate_state(2, dtype=np.uint32)
    )
...
def build_manifest(sensor_names: list[str], training_task_seeds: set[int]) -> dict[str, Any]:
    sides = np.asarray(["left"] * 10 + ["right"] * 10, dtype=object)
    side_seed = derive_seed(0, SIDE_STREAM_ID)["seed_u64"]
    np.random.default_rng(side_seed).shuffle(sides)
    rows = []
...
def validate_manifest(
    ...
    if len(rows) != INSTANCE_COUNT:
        raise PactPlaceEvalContractError("exactly 20 rows are required")
    if Counter(row["intrusion_side"] for row in rows) != {"left": 10, "right": 10}:
