#!/usr/bin/env python3
"""Freeze the 720-row attempt-3 zero-shot policy schedule."""
...
BOOTSTRAP_SEED = 2026080702
SCHEDULE_SEED = 2026080703


def token_plan_row_map() -> list[int]:
    """Select 40 frozen token-plan rows, paired identically across conditions."""
    ranked = sorted(
        range(100),
        key=lambda index: sha256_payload(
            {"schedule_seed": SCHEDULE_SEED, "token_plan_row": index}
        ),
    )
    return ranked[:40]
...
def build(
    *,
    ...
    surviving = list(manifest["conditions"])
    ...
    if not set(surviving).issubset(set(expert_screen["surviving_condition_ids"])):
        raise ValueError("v3 conditions were not authorized by the reused expert screen")
    token_rows = token_plan_row_map()
    rows = []
    for instance in manifest["rows"]:
        ...
        for position, (seed, arm) in enumerate(order):
            ...
            identity = {
                "schedule_schema": "pact_geometry_generalization_v3_schedule",
            ...
            row: dict[str, Any] = {
                ...
                **model,
                ...
                "token_plan_sha256": token_sha if arm == "PACT_PERMUTED" else None,
                "token_plan_row": token_rows[instance_index] if arm == "PACT_PERMUTED" else None,
    ...
    document: dict[str, Any] = {
        "schema_version": "pact_geometry_generalization_v3_schedule",
        "manifest_sha256": manifest["manifest_sha256"],
        ...
        "token_plan_row_map_by_instance_index": token_rows,
        "token_plan_pairing": "same frozen token-plan row for the same instance index in every condition and seed",
        ...
        "c0_reproduction_rule": {
            ...
            "ci_exclusion_of_zero_not_required_at_40_instances": True,
            "power_only_expected_C0_ci_half_width_pp": 7.35,
        },
        "decision_rule": {
