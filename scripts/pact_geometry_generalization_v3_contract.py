#!/usr/bin/env python3
"""Frozen policy-instance contract for geometry generalization attempt 3."""
...
SCHEMA_VERSION = "pact_geometry_generalization_v3_manifest"
ENVIRONMENT_VERSION = "pact_collision_corridor_geometry_generalization_v3"
MASTER_SEED = 2026080701
...
class GeometryV3ContractError(ValueError):
    pass
...
def _side_order() -> list[str]:
    values = np.asarray(["left"] * 20 + ["right"] * 20, dtype=object)
...
def build_manifest(
    *,
    ...
    surviving = set(expert_screen.get("surviving_condition_ids", []))
    if not set(CONDITIONS).issubset(surviving) or expert_screen.get("continue_to_policy_evaluation") is not True:
        raise GeometryV3ContractError("reused Phase 0 did not authorize every v3 condition")
    document: dict[str, Any] = {
        ...
        "phase0_bindings": {
            ...
            "Z_093_clean": [11, 12],
            "dropped_HALF_Y_030_clean": [11, 12],
            "phase0_reused_without_rescreening": True,
            "v2_expert_screen_surviving_condition_ids": list(expert_screen["surviving_condition_ids"]),
        },
...
def validate_manifest(document: dict[str, Any]) -> None:
    ...
    if observed != sha256_payload(payload):
        raise GeometryV3ContractError("manifest self-hash mismatch")
    if document.get("schema_version") != SCHEMA_VERSION:
        raise GeometryV3ContractError("schema changed")
    if document.get("conditions") != CONDITIONS or document.get("training_support") != TRAINING_SUPPORT:
        raise GeometryV3ContractError("condition contract changed")
    rows = document.get("rows", [])
    if len(rows) != 120 or len(document.get("sensor_names", [])) != 40:
        raise GeometryV3ContractError("row or sensor count changed")
    if len({row["episode_id"] for row in rows}) != 120:
        raise GeometryV3ContractError("duplicate episode IDs")
    for condition_id, condition in CONDITIONS.items():
        cell = [row for row in rows if row["condition_id"] == condition_id]
        if len(cell) != 40:
            raise GeometryV3ContractError("condition cell count changed")
        sides = Counter(row["intrusion_side"] for row in cell)
        if sides != {"left": 20, "right": 20}:
            raise GeometryV3ContractError("condition side balance changed")
        for row in cell:
            row_payload = dict(row)
            row_hash = row_payload.pop("row_sha256", None)
            if row_hash != sha256_payload(row_payload):
                raise GeometryV3ContractError("row self-hash mismatch")
            geometry = row["realized_geometry"]
            for axis in condition["moved_axes"]:
                if not _outside(float(geometry[axis]), TRAINING_SUPPORT[axis]):
                    raise GeometryV3ContractError(f"{condition_id} {axis} is inside training support")
            ...
            for axis in unchanged:
                ...
                if not low <= float(geometry[axis]) <= high:
                    raise GeometryV3ContractError(f"{condition_id} unexpectedly shifted {axis}")
    if Counter(row["intrusion_side"] for row in rows) != {"left": 60, "right": 60}:
        raise GeometryV3ContractError("overall side balance changed")
    for instance_index in range(40):
        ...
        if len(task_seeds) != 1:
            raise GeometryV3ContractError("conditions do not share fresh task instances")
...
def validate_recovery_event(path: str | Path, **kwargs: Any) -> dict[str, Any]:
    try:
        ...
        raise GeometryV3ContractError(str(error)) from error
