...
# v1010: V10.10 four-object (215 eps, table_camera + wrist). 24 cells, horizon 1050.
# table_camera = hybrid exo_camera_1 pose at fovy 58 (batman publish render). The old rename
# rendered it at 45 (zoomed in); TABLE_CAMERA_FOV=45 reproduces that (dir tag _fov45). README §0.1.
# One run. Knobs below; EXP / SEED / NUM_ROLLOUTS / SEED_BASE / HISTORY / TAG can be set from the
# environment (EXP=ACT ./scripts/exp/eval_v1010.sh). Protocol = eval_act_v1011d.py (fast path) with the
# v1010 world swapped in by eval_act_place.py. Skips a finished dir; refuses a partial one (no resume).
set -e
