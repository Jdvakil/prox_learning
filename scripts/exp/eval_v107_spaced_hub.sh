#!/usr/bin/env bash
# ./scripts/exp/eval_v107_spaced_hub.sh
# v107_spaced hub (200 eps, exo + wrist). Not the batman 210-ep T-107 set (eval_v107_spaced.sh).
# One run. Knobs below; EXP / SEED / NUM_ROLLOUTS / SEED_BASE / HISTORY / TAG can be set from the
# environment (EXP=ACT ./scripts/exp/eval_v107_spaced_hub.sh). Protocol = eval_act_v1011d.py (fast path) with the
