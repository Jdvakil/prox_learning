#!/usr/bin/env python3
"""Frozen analysis for held-out PACT geometry generalization."""
...
CONTRASTS = (
    ...
    ("PACT_PERMUTED", "ACT", "architecture"),
    ("PACT", "ACT", "combined"),
)
TOKENS = {
...
def load_matrix(schedule: dict[str, Any], output_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    expected_cells = len(schedule["surviving_condition_ids"]) * 25 * len(SEEDS) * len(ARMS)
    valid_cells = sum(
        len(arms)
...
def analyze(schedule: dict[str, Any], output_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if not reconciliation["reconciled"]:
        ...
        analysis = {"schema_version": "pact_geometry_generalization_analysis_v1", **base, "results_available": False}
        final = {
    ...
    for condition_index, condition in enumerate(schedule["surviving_condition_ids"]):
        ...
        for arm in ARMS:
            rows = [matrix[condition][instance][seed][arm] for instance in range(25) for seed in SEEDS]
        ...
        for seed_index, seed in enumerate(SEEDS):
            ...
            for arm in ARMS:
                seed_record["absolute"][arm] = summarize(
                    [matrix[condition][instance][seed][arm] for instance in range(25)]
                )
    ...
    analysis: dict[str, Any] = {
        "schema_version": "pact_geometry_generalization_analysis_v1",
        **base,
...
def render_report(schedule: dict[str, Any], analysis: dict[str, Any], final: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "This zero-shot evaluation uses the frozen ACT, PACT, and PACT_PERMUTED checkpoints; no policy, encoder, threshold, scene outcome, or endpoint was retuned.",
        "",
        "## Decision",
        "",
    ...
    if analysis.get("results_available"):
        ...
        lines += [
            ...
            "",
            "Full absolute arm performance, seed-unpooled contrasts, collision-free task success, manipulation success, entries, penetration, failure taxonomy, counts, Wilson intervals, and all 20,000-replicate bootstrap intervals are in `diagnostics_output/pact_geometry_generalization/analysis.json`.",
        ]
    else:
