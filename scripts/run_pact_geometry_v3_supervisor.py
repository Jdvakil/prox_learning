#!/usr/bin/env python3
"""Durable twelve-worker attempt-3 supervisor with a frozen batch watchdog."""
...
class GeometryV3Supervisor(v1.GeometrySupervisor):
    def __init__(
        self,
        *,
        schedule_path: Path,
        ...
        if screen.canonical_hash(payload) != observed:
            raise RuntimeError("attempt-3 geometry schedule self-hash mismatch")
        surviving = self.schedule.get("surviving_condition_ids", [])
        if (
            self.schedule.get("schema_version") != "pact_geometry_generalization_v3_schedule"
            or self.schedule.get("workers") != WORKERS
            or self.schedule.get("rollouts") != 720
            or len(self.schedule.get("rows", [])) != 720
            or {row["arm"] for row in self.schedule["rows"]} != ARMS
            or set(self.schedule.get("checkpoint_seeds", [])) != {3101, 3102, 3103}
            or surviving != ["C0", "C2", "Z_093"]
        ):
            raise RuntimeError("attempt-3 geometry schedule design mismatch")
        ...
        if self.contract.get("schema_version") != "pact_geometry_generalization_v3_dispatch":
            raise RuntimeError("wrong attempt-3 geometry dispatch contract")
        ...
        for label, record in self.contract["frozen_inputs"]["runtime"].items():
            if screen.base.sha256_file(Path(record["path"])) != record["sha256"]:
                raise RuntimeError(f"frozen attempt-3 runtime changed: {label}")
    ...
    def _watchdog_event(self, kind: str, rows: list[dict], **extra) -> Path:
        ...
        event = {
            "schema_version": "pact_geometry_generalization_v3_watchdog_event",
    ...
    def run(self) -> int:
        ...
        pid_record = {
            "schema_version": "pact_geometry_generalization_v3_supervisor_pid",
            "pid": os.getpid(),
            "identity": identity,
            "mode": self.mode,
            "schedule_sha256": self.schedule["schedule_sha256"],
        ...
        target = 1 if self.mode == "smoke" else 720
        ...
        if self.mode == "smoke":
            self._write_smoke()
        summary = {
            "schema_version": "pact_geometry_generalization_v3_execution",
            "schedule_sha256": self.schedule["schedule_sha256"],
