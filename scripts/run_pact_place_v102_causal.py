#!/usr/bin/env python3
"""V10.2 frozen-qpos causal proximity on the 12-row raised-pendant review pack.

Replays retained qpos without ``env.step``. Preserves the V10.1 raw causal
floors separately for inbound and outbound. Does not infer Phase-0 approval.
"""
...
from pact_place_v10_compound_pendant_contract import SCENE_XML_RELATIVE  # noqa: E402
...
DEFAULT_REVIEW_ROOT = ROOT / "diagnostics_output" / "pact_place_v102_review"
DEFAULT_OUTPUT_ROOT = ROOT / "diagnostics_output" / "pact_place_v102_causal"
...
def _run_cell(job: dict[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    scratch = Path(tempfile.mkdtemp(prefix="pact_v102_causal_"))
    task = sampler = None
    try:
        config = _make_config(
            scratch / "dummy.json",
        ...
        if task is None:
            raise RuntimeError("V10.2 sample_task returned None")
        task.reset()
        ...
        if len(sensor_names) != 40 or len(set(sensor_names)) != 40:
            raise RuntimeError("V10.2 requires the frozen 40-sensor suite")
...
def evaluate_causal(review_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    for key in required:
        ...
        if not is_v102_clean_success(result):
            failures.append(
                {
    ...
    artifact = {
        "schema_version": "pact_place_v102_causal_v1",
