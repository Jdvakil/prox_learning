#!/usr/bin/env python3
"""Collect 100 balanced V10.11d demonstrations; keep failed rows too.

Planner is PactPlaceCorridorPolicy (same official V10.11d expert). Sampler is
PactPlaceCorridorV1011DRandomizedLayoutSampler. Accepted rows are strict-clean
hybrid-camera episodes. Scientific rejects keep H5/mp4s in rows/ and are not
pruned.
"""
...
import pact_place_v1011d_n100_collection_contract as contract  # noqa: E402
...
SMOKE_COLLECTION_ROOT = ROOT / "diagnostics_output/pact_place_v1011d_n100_smoke"
SMOKE_DATASET_ROOT = ROOT / "output/pact_place_corridor_v10_11d_n100_smoke"
...
def _upstream_preflight() -> dict[str, Any]:
    ...
    try:
        ...
        config = _make_config(
            ROOT / "diagnostics_output/pact_place_v1011d_n100/_preflight_config.json",
            scene_xml=ROOT / contract.SCENE_BY_POSE["center"]["relative"],
            sampler_class=contract.SAMPLER_CLASS,
        )
        policy_name = config.policy_config.policy_cls.__name__
        sampler_name = config.task_sampler_config.task_sampler_class.__name__
        binding = {
            "policy_class": policy_name,
            "sampler_class": sampler_name,
            "environment_version": contract.ENVIRONMENT_VERSION,
        }
        if policy_name != contract.POLICY_CLASS:
            problems.append(f"policy {policy_name} != {contract.POLICY_CLASS}")
        if sampler_name != contract.SAMPLER_CLASS:
            problems.append(f"sampler {sampler_name} != {contract.SAMPLER_CLASS}")
        if contract.ENVIRONMENT_VERSION != (
            "pact_place_corridor_v10_11d_randomized_clutter"
        ):
            problems.append(f"env {contract.ENVIRONMENT_VERSION}")
    except Exception as error:  # noqa: BLE001
        problems.append(f"binding:{type(error).__name__}:{error}")
    return {
        "passed": not problems,
        "problems": problems,
        "upstream": upstream,
...
def worker(payload: dict[str, Any]) -> dict[str, Any]:
    """Use the official V10.11d expert plus the hybrid-camera publisher."""
...
def _status_payload(
    *, accepted: dict[str, int], attempted: dict[str, int], started: float,
    ...
    return {
        "schema_version": "pact_place_v1011d_mac_inspect_6_status_v1",
        "updated_utc": utc_now(),
...
def main() -> int:
    ...
    parser.add_argument("--gpus", type=int, default=contract.DEFAULT_GPUS)
    ...
    submit_index = 0
    print(
        f"V10.11d n100: {sum(accepted.values())}/{contract.TARGET_SUCCESSES} "
        f"accepted, {sum(attempted.values())} attempts, {args.workers} workers, "
        f"{args.gpus} gpus, policy={contract.POLICY_CLASS}, "
        ...
        while True:
            ...
            for future in done:
                ...
                print(json.dumps({
                    ...
                    "row_dir": Path(payload["row_dir"]).name,
    ...
    closeout = {
        **empty_authorization(),
        "schema_version": "pact_place_v1011d_collection_100_closeout_v1",
        "contract_version": contract.CONTRACT_VERSION,
