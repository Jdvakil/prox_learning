"""Expand seed 3103 to its original full 50-pair manifest, preserving prior results."""
...
EXT = WORK / 'amendments/seed3103_full50_20260908'
ROLE = 'final_3103_full50'
...
def full_jobs():
    jobs = BASE_JOBS('final_3103', 3103, 100)
    for job in jobs:
    ...
    assert all(j['schedule']['checkpoint_seed'] == 3103 and j['schedule']['averaging_history'] == 100 for j in jobs)
    return jobs
...
def install():
    ...
    def jobs(role, seed, history):
        assert (role, seed, history) == (ROLE, 3103, 100)
        return full_jobs()
...
def verify_prior_jobs(jobs):
    ...
    prior = read(WORK / 'evaluation/final_3103_h100_schedule.json')
    assert len(prior) == 34
    for job in prior:
...
def remaining_eta(jobs):
    ...
    # Recent completed seed-3104 extension, counted once per canonical rollout.
    records = [r for r in lines(WORK / 'evaluation/final_3104_full50_h100_ledger.jsonl')
    ...
    batches = math.ceil(pending / 12)
    estimate = batches * mean + 120
    clock_now = time.time()
    last_launch = clock_now + max(0, batches - 1) * mean
    result = {'utc': now(), 'additional_rollouts_pending': pending,
              ...
              'available_hours': (EXTENDED_DEADLINE - clock_now) / 3600,
              'estimated_last_launch_utc': datetime.fromtimestamp(last_launch, timezone.utc).isoformat(),
              'launch_stop_utc': datetime.fromtimestamp(LAUNCH_STOP, timezone.utc).isoformat(),
              'hard_deadline_utc': datetime.fromtimestamp(EXTENDED_DEADLINE, timezone.utc).isoformat()}
    append(EXT / 'eta.jsonl', result)
    if (pending and last_launch >= LAUNCH_STOP) or clock_now + estimate >= EXTENDED_DEADLINE:
        raise recovery.capacity.recovery.original.Paused(f'Extension cannot launch all jobs before the stop and finish before the hard deadline: {result}')
    return result


def write_report(analysis, verdict, reason, rows):
    old_ids = {r['episode_id'] for r in read(WORK / 'evaluation/final_3103_h100_complete.json')['rows']}
    text = ['# Seed 3103 — full 50-pair evaluation', '', f'Verdict: **{verdict}**.', '', reason, '',
            f'Updated: {now()}. ACT and PACT use their existing 60,000-update checkpoints, history 100, and the original frozen final-3103 manifest.', '']
    if rows:
        ...
        atomic(EXT / 'analysis.json', {**empty_authorization(), 'verdict': verdict, 'seed': 3103,
               'groups': summaries, 'paired': paired, 'benchmark': benchmark, 'rows': rows})
    text += ['', 'The user selected seed 3103 for expansion after reviewing its initial 17-pair result. This is a follow-up on that selected seed; the earlier three-seed report remains unchanged. The original development gate remains missed.', '',
...
def supervise(installed):
    ...
    try:
        run.stage('seed3103_full50_extension')
        ...
        remaining_eta(jobs)
        rows = run.run_eval(ROLE, 3103, 100)
        assert len(rows) == 100 and {r['seed'] for r in rows} == {3103}
        assert {r['episode_id'] for r in rows} == {r['episode_id'] for r in read(WORK / 'manifests/final_3103.json')['rows']}
