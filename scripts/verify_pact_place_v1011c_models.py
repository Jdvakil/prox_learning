...
from pact_place_v1011c_experiment import DATA, TRAIN
CONVERTED_DATASET_ROOT = str(DATA.relative_to(ROOT))
TRAINING_ROOT = str(TRAIN)
...
def verify_arm(arm: str, split: dict[str, Any], seed: int) -> dict[str, Any]:
    import torch  # noqa: PLC0415
    from policy import ACTPolicy  # noqa: PLC0415

    problems: list[str] = []
    directory = Path(TRAINING_ROOT) / f"{arm}_seed{seed}"
    ...
    with detr_argv(str(directory), seed):
