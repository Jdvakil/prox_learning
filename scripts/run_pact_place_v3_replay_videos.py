#!/usr/bin/env python3
"""Replay-render the 24 attempt-3 place-corridor episodes from recorded qpos.

Restore `data.qpos`, call `mj_forward`, and render. Do not step physics, do not
run the expert, and do not modify Phase-0 artifacts.
"""
...
SCREEN_ROOT = ROOT / "diagnostics_output/pact_place_corridor_v3"
DEFAULT_OUTPUT = ROOT / "diagnostics_output/pact_place_corridor_v3_videos"
V3_CONFIG_SHA256 = (
...
FAIL_CLIP_STEMS = {
    2: "row02_FAIL_outbound_approach_cup_lost",
    6: "row06_FAIL_placement_descent_not_released",
    9: "row09_FAIL_outbound_approach_tracking_12.6cm",
    12: "row12_FAIL_pregrasp_never_grasped",
    20: "row20_FAIL_lift_cup_dropped",
    21: "row21_FAIL_outbound_pass_cup_lost",
}
...
CRIB_FAILURES = (
    {
        ...
        "steps": 14,
        "phase": "pregrasp",
        "recorded": "gripper stayed open 84.0 mm; tracking error 10.5 cm",
        "watch_for": "Dies in under half a second. Does the arm even start toward the cup?",
    },
    {
        "row": 20,
        "steps": 133,
        "phase": "lift",
        "recorded": "cup ended z 0.753 vs 0.794 start; gripper emptied",
        "watch_for": "Grasp closes but slips — does the cup get knocked off the shelf?",
    },
    {
        "row": 2,
        "steps": 193,
        "phase": "outbound_approach",
        "recorded": "lifted 1 cm then gripper → 0.000; tracking error only 9.5 mm",
        "watch_for": "Cup lost while tracking fine. Where does it slip out?",
    },
    {
        "row": 21,
        "steps": 281,
        "phase": "outbound_pass",
        "recorded": "gripper → 0.000; cup abandoned at x 0.72",
        "watch_for": "Same class as row 2 but later — does it clip the aperture?",
    },
    {
        "row": 9,
        "steps": 234,
        "phase": "outbound_approach",
        "recorded": (
            "tracking error **12.58 cm**; actual z 0.9866 vs target 0.9062, "
            "plus a 9.4 cm y shortfall"
        ),
        "watch_for": "The claim is the arm climbs out of the carry plane. Does it visibly lift?",
    },
    {
        "row": 6,
        "steps": 388,
        "phase": "placement_descent",
        "recorded": (
            "cup on the tray, `supported_by_receptacle` true, `robot_contact` "
            "still true, place error 20.9 mm"
        ),
        "watch_for": (
            "The release defect recurring once. Is the cup actually placed and "
            "the gripper just still on it?"
        ),
    },
)

FAIL_RECORD_PATHS = (
    ROOT / "diagnostics_output/pact_place_corridor/expert_screen.json",
    ROOT / "diagnostics_output/pact_place_corridor/stop_record.json",
    ROOT / "diagnostics_output/pact_place_corridor_v2/expert_screen.json",
    ROOT / "diagnostics_output/pact_place_corridor_v2/stop_record.json",
    ROOT / "diagnostics_output/pact_place_corridor_v3/expert_screen.json",
    ROOT / "diagnostics_output/pact_place_corridor_v3/stop_record.json",
)
...
def hazard_bar_active(env) -> bool:
    ...
    return any(
        ...
        for pair in robot_environment_contact_pairs(env)
    )
...
def _prepare_task(row: dict[str, Any], selected_seed: dict[str, int]):
    ...
    scratch = Path(tempfile.mkdtemp(prefix="pact_place_v3_replay_"))
    task = sampler = None
    try:
...
def render_row(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if result.get("config_sha256") != V3_CONFIG_SHA256:
        raise RuntimeError(f"row {role_index} is not the frozen v3 screen")
    payload = dict(result)
...
def main() -> int:
    ...
    if contract["config_sha256"] != V3_CONFIG_SHA256:
        raise SystemExit("v3 contract hash changed")
    ...
    manifest = {
        "schema_version": "pact_place_v3_replay_videos_v1",
        ...
        "v2_hazard_rows_6_12": "out_of_scope",
