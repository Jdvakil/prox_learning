#!/usr/bin/env python3
"""Freeze the zero-shot geometry-generalization policy schedule."""
        ...
        for arm in ARMS:
            ...
            for path_key, hash_key in (
                ...
                if path is not None and sha256_file(path) != expected:
                    raise ValueError(f"frozen artifact changed: {seed} {arm} {path}")
...
def condition_order(instance_index: int, condition_index: int) -> list[tuple[int, str]]:
    ...
    for seed in seed_order:
        offset = (instance_index + condition_index + SEEDS.index(seed)) % len(arm_base)
        arms = arm_base[offset:] + arm_base[:offset]
...
def build(
    *,
    ...
    if expert_screen.get("manifest_sha256") != manifest["manifest_sha256"]:
        raise ValueError("expert screen belongs to another manifest")
    ...
    if "C0" not in surviving or len([item for item in surviving if item != "C0"]) < 2:
        raise ValueError("C0 and at least two shifted conditions must survive")
    rows = []
    instances = [row for row in manifest["rows"] if row["condition_id"] in surviving]
    for instance in instances:
        ...
        for position, (seed, arm) in enumerate(order):
            ...
            identity = {
                "schedule_schema": "pact_geometry_generalization_schedule_v1",
            ...
            row: dict[str, Any] = {
                ...
                **model,
                ...
                "token_plan_sha256": token_sha if arm == "PACT_PERMUTED" else None,
                "token_plan_row": (
                    condition_index * 25 + instance_index if arm == "PACT_PERMUTED" else None
                ),
    ...
    document: dict[str, Any] = {
        "schema_version": "pact_geometry_generalization_schedule_v1",
        "manifest_sha256": manifest["manifest_sha256"],
