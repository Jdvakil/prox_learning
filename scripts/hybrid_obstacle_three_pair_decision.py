...
ALLOWED = (
    "THREE_PAIR_JOINT_GATE_READY_FOR_CONFIRMATORY_41",
...
def main() -> int:
    ...
    for name in ("provenance", "reproduction", "calibration", "manifest", "partition",
                 "previous-decision", "out"):
        ap.add_argument(f"--{name}", required=True, type=Path)
    args = ap.parse_args()
    for field in ("provenance", "reproduction", "calibration", "manifest", "partition",
                  "previous_decision", "out"):
    ...
    previous = json.loads(args.previous_decision.read_text())
    manifest = json.loads(args.manifest.read_text())
    partition = json.loads(args.partition.read_text())

    temporal = calibration["temporal_classification"]
    historical_passes = calibration["historical_regression"]["passes"]
    if not provenance["all_matched"]:
        decision, case = "CHECKPOINT_OR_SOURCE_MISMATCH", None
    elif not reproduction["reproduced"]:
        decision, case = "THREE_PAIR_AGREEMENT_REPRODUCTION_FAILED", None
    elif not calibration.get("feasible"):
        decision, case = "THREE_PAIR_JOINT_GATE_CALIBRATION_INFEASIBLE", "B"
    elif not calibration["inference_stability"]["stable"]:
        decision, case = "REFERENCE_MODEL_INFERENCE_UNSTABLE", None
    elif historical_passes and temporal["only_temporal_failures"]:
        # every coverage, rate and acceptance gate passed and the only residual failures
        # are clustering or post-support persistence
        decision, case = "THREE_PAIR_TEMPORAL_CLUSTERING_REMAINS", "C"
    elif not (calibration["nested_passed"] and historical_passes
              and calibration["reused_diagnostic"]["passed"]):
        # the historical regression failing is a per-frame decision failure, not a
        # clustering artifact, so it cannot be reported as a temporal-only outcome
        decision, case = "THREE_PAIR_JOINT_GATE_OFFLINE_TRANSFER_FAILED", "D"
    else:
        decision, case = "THREE_PAIR_JOINT_GATE_TASK_INCOMPLETE", None
    ...
    payload = {
        ...
        "case": case,
        "case_interpretation": {
            "A": "reproduction, calibration, offline transfer and live gates all pass",
            "B": "no calibration pair satisfies the unchanged contract",
            "C": ("coverage and aggregate false activation are solved; only clustered "
                  "false activation or post-support persistence remains, authorizing a "
                  "separate temporal-gating study"),
            "D": ("another offline transfer failure: the historical regression fails on "
                  "per-frame decisions, which is not a temporal artifact and cannot be "
                  "deferred to a temporal study"),
        }.get(case),
        "previous_decision": "FULL_SEED_JOINT_GATE_OFFLINE_TRANSFER_FAILED",
        "reproduction": reproduction,
        "temporal_classification": temporal,
        "metric_substitution": {
            "old_controlling": "anchor_agreement = mean(J01, J02)",
            "new_controlling": "full_pairwise_agreement = mean(J01, J02, J12)",
            "handoff_premise": ("the metric substitution reverses the historical "
                                "decision"),
            "premise_supported": False,
            "historical_executed_under_anchor":
                previous["historical_regression"]["executed"],
            "historical_executed_under_three_pair":
                calibration["historical_regression"]["executed"],
            "net_historical_decisions_changed": abs(
                previous["historical_regression"]["executed"]
                - calibration["historical_regression"]["executed"]),
            "why": ("J12 is exactly 0.000 on all 17 historical frames, so restoring it "
                    "multiplies every one of them by the same 2/3 factor and adds no "
                    "discriminative signal among them. The calibrated threshold shifted "
                    "down by a comparable factor (0.225 -> 0.166667, ratio 0.74 against "
                    "the 2/3 rescale), so their position relative to the threshold is "
                    "almost unchanged"),
            "previous_agreement_threshold":
                previous["calibration"]["selected_agreement_threshold"],
            "new_agreement_threshold":
                calibration["selected"]["agreement_threshold"],
        },
        "mode": MODE,
