#!/usr/bin/env python3
"""Replay-render the 24 attempt-6c place-corridor episodes from recorded qpos.

Restore `data.qpos`, call `mj_forward`, and render. Do not step physics, do not
run the expert, and do not modify Phase-0 artifacts. Copied from the v6b
renderer. Do not edit the v5 renderer. Do not edit the v6b renderer.
"""
...
SCREEN_ROOT = ROOT / "diagnostics_output/pact_place_corridor_v6c"
DEFAULT_OUTPUT = ROOT / "diagnostics_output/pact_place_corridor_v6c_videos"
V6C_CONFIG_SHA256 = (
...
FAIL_CLIP_STEMS = {
    7: "row07_FAIL_outbound_approach_ik_cascade",
}
...
FAIL_RECORD_PATHS = (
    ...
    ROOT / "diagnostics_output/pact_place_corridor_v6c/expert_screen.json",
)
...
def _assert_static_furniture(task, row: dict[str, Any]) -> None:
    ...
    slots = cfg["phase0_gate"]["clutter_sweep_v6c"]["chosen_slots_xy_m"]
    height = float(cfg["phase0_gate"]["clutter_sweep_v6c"]["chosen_height_m"])
...
def _prepare_task(row: dict[str, Any], selected_seed: dict[str, int]):
    ...
    scratch = Path(tempfile.mkdtemp(prefix="pact_place_v6c_replay_"))
    task = sampler = None
    try:
        scene_xml = ROOT / json.loads(CONFIG_PATH.read_text())["scene"]["xml"]
        if scene_xml.name != REQUIRED_SCENE_XML:
            raise RuntimeError(
                f"v6c must render {REQUIRED_SCENE_XML}, got {scene_xml.name}"
            )
...
def render_row(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if result.get("config_sha256") != V6C_CONFIG_SHA256:
        raise RuntimeError(f"row {role_index} is not the frozen v6c screen")
    payload = dict(result)
...
def main() -> int:
    ...
    manifest = {
        "schema_version": "pact_place_v6c_replay_videos_v1",
