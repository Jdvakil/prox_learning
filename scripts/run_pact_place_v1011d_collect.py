#!/usr/bin/env python3
"""Collect the V10.11d Mac inspect set (6 hybrid-camera attempts).

Uses the official V10.11d owner-review expert (PactPlaceCorridorPolicy on the
review stream/seed that already produced 3 clean successes) plus the V10.10
tablecam publisher.  Accepted rows contain wrist RGB, table-camera RGB,
per-frame table-camera calibration, and all 40 raw proximity cameras.  Mac
inspect keeps rejected rollouts so schema and clutter can be reviewed before
batman collection.
"""
...
import pact_place_v1011d_collection_contract as contract  # noqa: E402
...
SMOKE_COLLECTION_ROOT = ROOT / "diagnostics_output/pact_place_v1011d_mac_inspect_6"
SMOKE_DATASET_ROOT = ROOT / "output/pact_place_corridor_v10_11d_mac_inspect_6"
...
def worker(payload: dict[str, Any]) -> dict[str, Any]:
    """Same Mac inspect path: PactPlaceCorridorPolicy + V10.11d sampler."""
...
def main() -> int:
    ...
    submit_index = 0
    print(
        f"V10.11d collection: {sum(accepted.values())}/{contract.TARGET_SUCCESSES} "
