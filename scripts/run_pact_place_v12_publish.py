#!/usr/bin/env python3
"""v12 episode publish / validate helpers.

Uses the full datagen pipeline (prepare_episode_for_saving /
save_trajectories) so every accepted episode carries actions, qpos/qvel,
wrist RGB and all 40 proximity sensors.
"""
...
from pact_place_v12_contract import (  # noqa: E402
    COLLECTION_ROOT,
    CONTRACT_VERSION_V12 as CONTRACT_VERSION_V108,
    DATASET_ROOT,
    ENVIRONMENT_VERSION,
    ...
    WRIST_VIDEO_SUFFIX,
    build_contract,
    build_row as build_attempt_row,
    cell_key,
    cells,
    empty_authorization,
    row_defects,
...
def quotas() -> dict[str, int]:
    return {cell_key(*cell): 6 for cell in cells()}


def quota_totals() -> dict[str, int]:
    values = quotas()
    return {"cells": len(values), "target_successes": sum(values.values())}


def remaining_quota(accepted: dict[str, int]) -> dict[str, int]:
    return {key: max(0, 6 - int(accepted.get(key, 0))) for key in quotas()}


def quotas_met(accepted: dict[str, int]) -> bool:
    return all(int(accepted.get(key, 0)) >= 6 for key in quotas())


def next_attempts(*_args, **_kwargs) -> list[Any]:
    return []
...
def scene_preflight() -> dict[str, Any]:
    from pact_place_v12_contract import SCENE_BY_POSE, SCENE_RELATIVE

    path = ROOT / SCENE_RELATIVE
    problems: list[str] = []
    scenes: dict[str, Any] = {}
    if not path.is_file():
        problems.append(f"missing scene {SCENE_RELATIVE}")
        return {"scenes": scenes, "problems": problems, "passed": False}
    observed = sha256_file(path)
    for pose, entry in SCENE_BY_POSE().items():
        if observed != entry["sha256"]:
            problems.append(f"scene drifted {pose}")
        scenes[pose] = {"relative": SCENE_RELATIVE, "sha256": observed}
    return {
        "scenes": scenes,
        "problems": problems,
    ...
    try:
        ...
        for retry_index in range(int(row["max_sampling_retries"]) + 1):
            if retry_index == 0:
                ...
                from pact_place_v12_contract import cell_seed
...
def main() -> int:
    ...
    elapsed = time.time() - started
    summary = {
        "schema_version": "pact_place_v12_collection_v1",
