...
def replay_check(scene):
    ...
    if not all(comparison[k]['exact'] for k in ('model_output','arm','gripper')):
        diagnosis_path=C/'root_review/frozen_query_reconstruction.json'
        diagnosis=read(diagnosis_path)
        assert diagnosis['scene_id']==scene['scene_id'] and diagnosis['training_seed']==3103
        assert diagnosis['checkpoint_sha256']==result['checkpoint_sha256']==prior['checkpoint_sha256']
        assert sha(C/'root_review/reconstruct_frozen.py')==diagnosis['analysis_code_sha256']
        for path,expected in diagnosis['source_hashes'].items():
            assert sha(path)==expected,path
        assert diagnosis['repeated_inference_exact']
        assert all(diagnosis['comparisons'][name]['first_vector_exact'] for name in ('original','new'))
        assert diagnosis['predicted_delta']==diagnosis['retained_delta']
        for key in ('qpos_sha256','proximity_sha256'):
            assert diagnosis['inputs']['original'][key]==diagnosis['inputs']['new'][key]
        assert diagnosis['preprocessed_rgb_changed_channels']==3 and diagnosis['preprocessed_rgb_max_delta']==1
        assert read(directory/'initial_pairing.json')['passed']
        assert comparison['gripper']['exact']
        freeze(C/'frozen_replay_resolution.json',{'diagnosis_sha256':sha(diagnosis_path),
            'initial_pairing_sha256':sha(directory/'initial_pairing.json'),
            'action_trace_is_not_exact':True,
            'binding_accepted':'Identical non-RGB inputs; both retained first queries reproduced exactly from their own saved RGB. Full action traces are not claimed identical.'})
