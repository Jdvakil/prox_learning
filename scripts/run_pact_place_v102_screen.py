#!/usr/bin/env python3
"""V10.2 Step-2 six-row engineering screen.

One row per F0/F1/F2 family and side on stream
``pact-place-v10.2-raised-pendant-smoke``. Admission to the full review needs
all 6/6 rows clean under the V10.2 definition. Authorizes nothing.
"""
...
DEFAULT_OUTPUT_ROOT = ROOT / "diagnostics_output" / "pact_place_v102_screen"
DEFAULT_PREFLIGHT = (
    ...
    return render_v102_review_video(
        {
            ...
            "clip_label": (
                f"v10.2 screen {family} {row['intrusion_side']} {result.get('status')}"
            ),
...
def main() -> int:
    ...
    rows = list(contract["screen_rows"])
    if len(rows) != N_SCREEN_ROWS:
        raise RuntimeError("screen contract must contain exactly 6 rows")
    config = {
        "schema_version": "pact_place_v102_screen_config_v1",
    ...
    eligibility = screen_eligibility(rows, results)
    manifest = {
        "schema_version": "pact_place_v102_screen_manifest_v1",
        "contract_version": CONTRACT_VERSION,
        "contract_sha256": contract["contract_sha256"],
        "config_sha256": config["config_sha256"],
        "preflight_sha256": preflight.get("artifact_sha256"),
    ...
    digest = write_immutable(output_root / "screen_manifest.json", manifest)
    status_counts: dict[str, int] = {}
    for item in results:
        status_counts[str(item["status"])] = status_counts.get(str(item["status"]), 0) + 1
    summary = {
        "schema_version": "pact_place_v102_screen_summary_v1",
        "n_rows": N_SCREEN_ROWS,
    ...
    return 0 if eligibility["screen_passed"] else 1
