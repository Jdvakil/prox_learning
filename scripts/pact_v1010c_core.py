...
C = ROOT / 'diagnostics_output/pact_place_v1010c_readout_s3103'
UPSTREAM = C / 'upstream'
MAIN = UPSTREAM / 'prox_learning'
SCHEMA = 'pact_place_v1010c_readout_s3103_v1'
...
def make_policy():
    ...
    return original.make_policy('ACT', policy_config(), argv_guard=original._detr_safe_argv(
        'ACT', 'pact_v1010c_readout', 3103, 2000, str(C / 'checkpoint'))).cuda()
...
def make_loaders():
    ...
    train, val, stats, meta = load_pact_fixed_split_data(str(W / 'converted'), split,
        ['wrist_camera'], 8, 8, 100, seed=3103, num_workers=4, n_proximity_sensors=40,
