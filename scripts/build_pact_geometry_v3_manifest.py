#!/usr/bin/env python3
"""Build or verify the attempt-3 policy-evaluation manifest."""
...
from pact_geometry_generalization_v3_contract import (  # noqa: E402
    build_manifest,
    sha256_file,
    validate_manifest,
)
...
OUTPUT = ROOT / "configs/pact_geometry_generalization_v3.json"
...
SOURCE_FILES = {
    ...
    "dispatch_builder": ROOT / "scripts/freeze_pact_geometry_v3_dispatch.py",
    ...
    "analysis": ROOT / "scripts/analyze_pact_geometry_generalization_v3.py",
    ...
    "v3_preregistration": ROOT / "docs/PACT_GEOMETRY_GENERALIZATION_V3_PREREGISTRATION.md",
    "worker_sizing_builder": ROOT / "scripts/build_pact_geometry_v3_worker_sizing.py",
    "worker_sizing": ROOT / "diagnostics_output/pact_geometry_generalization_v3/worker_sizing.json",
    "v2_abandonment": ROOT / "diagnostics_output/pact_geometry_generalization_v2/abandonment.json",
}
...
def build() -> dict:
    ...
    from pact_geometry_generalization_v3_contract import sha256_payload
...
def main() -> int:
    ...
    if args.check:
        if not OUTPUT.exists() or json.loads(OUTPUT.read_text()) != document:
            print("attempt-3 policy manifest differs from deterministic regeneration")
            return 1
