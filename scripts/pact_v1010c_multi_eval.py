"""Fine-tuned or original PACT on the unchanged seed-specific physical evaluation."""
...
def run(job_path):
    ...
    exp=wrist.Wrist288EvalConfig(output_dir=OUTPUT,num_workers=1,
        policy_config=ReadoutPolicyConfig(arm='PACT',checkpoint_dir=str(checkpoint.parent),
            checkpoint_path=str(checkpoint),stats_path=str(stats),checkpoint_seed=TRAINING_SEED,
    ...
    try:
        ...
        result={'schema_version':SCHEMA,'status':'complete','arm':'PACT','episode_id':scene['scene_id'],
            ...
            'seed':selected,'checkpoint_seed':TRAINING_SEED,'checkpoint_sha256':job['checkpoint_sha256'],
            ...
            'policy_info':info,'scene_id':scene['scene_id'],'scene_block':TRAINING_SEED,'training_seed':TRAINING_SEED,
