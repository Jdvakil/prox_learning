#!/usr/bin/env python3
"""Run V10.11c's frozen validation sequence through owner review."""
...
import pact_place_v1011c_contract as contract  # noqa: E402

STATUS_ROOT = ROOT / "diagnostics_output/pact_place_v1011c_pipeline"
...
def _write_status(stage: str, *, complete: bool = False, error: str | None = None) -> None:
    ...
    STATUS_PATH.write_text(
        json.dumps(
            {
                "schema_version": "pact_place_v1011c_pipeline_status_v1",
                "stage": stage,
                "complete": complete,
                "error": error,
...
def main() -> int:
    if (ROOT / contract.REVIEW_ROOT / "review_manifest.json").exists():
        raise SystemExit("V10.11c review already exists; refusing a second pipeline run")
    try:
        _run(
            "targeted_tests",
            [
        ...
        _run(
            ...
            [sys.executable, "scripts/run_pact_place_v1011c_visibility.py"],
        )
        _run(
            "preflight_96",
            [sys.executable, "scripts/run_pact_place_v1011c_preflight.py", "--workers", "8"],
        )
        _run(
            "six_episode_review",
            [
                sys.executable,
                "scripts/run_pact_place_v1011c_review.py",
                "--workers",
                "12",
                "--render-workers",
                "3",
