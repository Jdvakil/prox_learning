#!/usr/bin/env python3
"""Launch detached attempt-3 evaluation, compaction, and throughput monitor."""
...
def main() -> int:
    ...
    if contract.get("schema_version") != "pact_geometry_generalization_v3_dispatch":
        raise ValueError("wrong attempt-3 geometry dispatch contract")
    ...
    if (output_root / "full_launcher_receipt.json").exists():
        raise ValueError("full attempt-3 geometry dispatch already launched")
    ...
    completed = subprocess.run(
        [
            str(PYTHON),
            str(ROOT / "scripts/launch_pact_geometry_v3_detached.py"),
            "--schedule",
    ...
    receipt = {
        "schema_version": "pact_geometry_generalization_v3_full_launcher_receipt",
