"""Expand seed 3105 to its original full 50-pair manifest, preserving prior results."""
...
EXT = WORK / 'amendments/seed3105_full50_20260907'
ROLE = 'final_3105_full50'
...
def full_jobs():
    jobs = BASE_JOBS('final_3105', 3105, 100)
    for job in jobs:
    ...
    assert all(j['schedule']['checkpoint_seed'] == 3105 and j['schedule']['averaging_history'] == 100 for j in jobs)
    return jobs
...
def install():
    ...
    def jobs(role, seed, history):
        assert (role, seed, history) == (ROLE, 3105, 100)
        return full_jobs()
...
def verify_prior_jobs(jobs):
    ...
    prior = read(WORK / 'evaluation/final_3105_h100_schedule.json')
    assert len(prior) == 32
    for job in prior:
...
def remaining_eta(jobs):
    ...
    records = [r for p in (WORK / 'evaluation').glob('final_*_h100_ledger.jsonl')
               for r in lines(p) if r.get('valid_completion')]
...
def write_report(analysis, verdict, reason, rows):
    old_ids = {r['episode_id'] for r in read(WORK / 'evaluation/final_3105_h100_complete.json')['rows']}
    text = ['# Seed 3105 — full 50-pair evaluation', '', f'Verdict: **{verdict}**.', '', reason, '',
            f'Updated: {now()}. ACT and PACT use their existing 60,000-update checkpoints, history 100, and the original frozen final-3105 manifest.', '']
    if rows:
        assert len(rows) == 100 and len({(r['episode_id'], r['arm']) for r in rows}) == 100
        groups = {'prior_16': [r for r in rows if r['episode_id'] in old_ids],
                  'added_34': [r for r in rows if r['episode_id'] not in old_ids], 'full_50': rows}
        ...
        text += ['| Instances | ACT success | PACT success | PACT−ACT | ACT collision-free success | PACT collision-free success |',
                 '|---|---:|---:|---:|---:|---:|']
        for group, label in [('prior_16', 'Previously completed 16'), ('added_34', 'Additional 34'), ('full_50', 'Full 50')]:
        ...
        atomic(EXT / 'analysis.json', {**empty_authorization(), 'verdict': verdict, 'seed': 3105,
               'groups': summaries, 'paired': paired, 'benchmark': benchmark, 'rows': rows})
    text += ['', 'The user selected seed 3105 for expansion after reviewing its initial 16-pair result. This is a follow-up on that selected seed; the earlier three-seed report remains unchanged. The original development gate remains missed.', '',
...
def supervise(installed):
    ...
    try:
        run.stage('seed3105_full50_extension')
        jobs = full_jobs();prior = verify_prior_jobs(jobs)
        freeze(EXT / 'adoption.json', {'prior_pairs': 16, 'prior_jobs_sha256': digest(prior),
               'full_jobs_sha256': digest(jobs), 'additional_pairs': 34, 'new_training': False})
        remaining_eta(jobs)
        rows = run.run_eval(ROLE, 3105, 100)
        assert len(rows) == 100 and {r['seed'] for r in rows} == {3105}
        assert {r['episode_id'] for r in rows} == {r['episode_id'] for r in read(WORK / 'manifests/final_3105.json')['rows']}
        verdict = 'SEED 3105 FULL 50-PAIR EVALUATION COMPLETE'
        reason = 'Completed 50 valid paired instances / 100 rollouts: 16 existing pairs adopted and 34 additional pairs evaluated.'
