...
def score(models, head, partition, episodes, device, hard_threshold, *, batch=256):
    ...
    for index in keep:
        ...
        for start in range(0, n, batch):
            ...
            agreement[slot] = anchor_mask_agreement(m0, m1, m2)
        ...
        out.append(Trajectory(
            ...
            activity, agreement, three, cosine, oracle_active[rows],
...
def main() -> int:
    ...
    deployment = {
        ...
        "agreement_definition": "mean(J(seed0,seed1), J(seed0,seed2)); empty masks agree",
    ...
    for t in diagnostic:
        for step in range(t.frames):
            ...
            historical_rows.append({
                ...
                "anchor_mask_agreement": float(t.agreement[step]),
                "agreement_threshold": U,
    ...
    with torch.no_grad():
        for _ in range(INFERENCE_REPEATS):
            ...
            runs.append({
                ...
                "agreement": anchor_mask_agreement(*masks),
                "j01": jaccard(masks[0], masks[1]),
                "j02": jaccard(masks[0], masks[2]),
                "execute": joint_decision(probs[0].reshape(256, -1).max(axis=1), A,
                                          anchor_mask_agreement(*masks), U)["execute"],
            })
    stability = {
        ...
        "jaccard_identical": bool(all(
            ...
            and np.array_equal(r["j02"], runs[0]["j02"]) for r in runs)),
    ...
    report = {
        "schema": "hybrid_obstacle_joint_gate_calibration_v1", "mode": MODE,
        "feasible": True,
