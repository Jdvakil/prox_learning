...
import pact_wrist280_seed3104_full50 as extension
...
def jobs(tmp_path):
    return [{'directory': str(tmp_path / f'{i}_{arm}'),
             ...
             'schedule': {'episode_id': str(i), 'rollout_id': f'{i}_{arm}', 'arm': arm,
                          'checkpoint_seed': 3104, 'averaging_history': 100}}
...
def test_adoption_requires_exact_prior_jobs_and_observed_zero_exits(tmp_path, monkeypatch):
    ...
    prior = all_jobs[:34]
    extension.atomic(tmp_path / 'evaluation/final_3104_h100_schedule.json', prior)
    for job in prior:
