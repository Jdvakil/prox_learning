"""V10.10 wrist288 evaluation: frozen histories, raw contact/pose evidence."""
...
class Wrist288ContactAudit(audit_module.PactPlaceContactAudit):
    def reset(self):
        super().reset()
        self.raw_times = []
        self.raw_steps = []
...
class Wrist288InferencePolicy(v109.PactPlaceV109InferencePolicy):
    def reset(self):
        super().reset()
        self._contact_audit = Wrist288ContactAudit()
        ...
        self._target_times = set()
        self._proximity_hook_calls = 0

    def prepare_model(self):
        super().prepare_model()
        if self.pc.arm == 'PACT' and not hasattr(self, '_prox_hook'):
            def consumed(module, inputs, output):
                assert tuple(inputs[0].shape[-2:]) == (40,32)
                assert __import__('torch').isfinite(inputs[0]).all()
                self._proximity_hook_calls += 1
            self._prox_hook = self._policy.model.input_proj_proximity.register_forward_hook(consumed)

    def inference_model(self, observation):
        # Base inference appends the current chunk; retain H-1 previous chunks.
        self._pending_chunks = self._pending_chunks[-(HISTORY-1):] if HISTORY > 1 else []
        if not self._initial_written:
            ...
            with h5py.File(initial,'x') as handle:
                ...
                for name in ('qpos','qvel','act','ctrl','mocap_pos','mocap_quat'):
                    write('physics/'+name,np.asarray(getattr(data,name)))
                for name in ('body_pos','body_quat','geom_pos','geom_quat','geom_size','geom_friction','jnt_range'):
                    write('model/'+name,np.asarray(getattr(model,name)))
                write('configuration/row',ACTIVE_ROW)
                write('configuration/sampler',collection.SAMPLER_CLASS)
                write('configuration/scene_sha256',ACTIVE_ROW['pact_v106_scene_sha256'])
                write('configuration/horizon',900)
                write('configuration/end_on_success',False)
                write('configuration/action_noise',False)
    ...
    def _v109_update_clutter_stability(self):
        ...
        target = manager.get_object_by_name(self.config.task_config.pickup_obj_name)
        target_time = round(float(data.time),12)
        if target_time not in self._target_times:
            self._target_times.add(target_time)
            self._target_positions.append((int(self._step), *map(float,target.position)))
    ...
    def get_info(self):
        ...
        if not file.exists():
            ...
            with h5py.File(file,'x') as h:
                h.attrs['schema_version'] = 'pact_wrist288_raw_telemetry_v1'
                ...
                for name,value in (
                    ('target/control_step_and_world_xyz',np.asarray(self._target_positions,dtype=np.float64)),
        ...
        if self.pc.arm == 'PACT':
            assert self._proximity_hook_calls == 900, self._proximity_hook_calls
        info.update({'averaging_history':HISTORY,'proximity_projection_calls':self._proximity_hook_calls,'sampler_class':collection.SAMPLER_CLASS,
...
class Wrist288PolicyConfig(place.PactPlacePolicyConfig):
    policy_cls: type = Wrist288InferencePolicy
    num_queries: int = 100


class Wrist288EvalConfig(place._BaseConfig):
    def __init__(self,*args,**kwargs):
        ...
        self.task_sampler_config.task_sampler_class = PactPlaceCorridorV1010FourObjectSampler
...
def sampler_for(row,default):
    ...
    return PactPlaceCorridorV1010FourObjectSampler
...
def main():
    global ACTIVE_ROW, OUTPUT, HISTORY
    pin_torch()
    ...
    parser.add_argument('--save-trajectory',action='store_true',required=True)
    parser.add_argument('--averaging-history',type=int,choices=(100,10),required=True)
    ...
    sys.argv.remove('--save-trajectory')
    pos=sys.argv.index('--averaging-history'); del sys.argv[pos:pos+2]
    sys.argv.append('--save-video')
    HISTORY=known.averaging_history
    ...
    scene = ROOT / ACTIVE_ROW['pact_v1010_scene_relative']
    assert sha256_file(scene) == ACTIVE_ROW['pact_v106_scene_sha256']
    module_path = Path(inspect.getfile(PactPlaceCorridorV1010FourObjectSampler)).resolve()
    assert module_path == ROOT / 'submodules/molmospaces/molmo_spaces/tasks/enclosure_reach.py'
    source = read(WORK / 'config.json')
    assert sha256_file(module_path) == source['file_hashes'][str(module_path.relative_to(ROOT))]
    ...
    source_main=inspect.getsource(legacy.main)
    assert source_main.count('policy_best.ckpt')==1
    exec(compile(source_main.replace('policy_best.ckpt','policy_update_60000.ckpt'),legacy.__file__,'exec'),legacy.__dict__)
    v109.install_v109_bindings()
    place.PactPlaceInferencePolicy = Wrist288InferencePolicy
    place.policy_config_factory = lambda **kw:Wrist288PolicyConfig(num_queries=100,**kw)
    place.PactPlaceEvalConfig = Wrist288EvalConfig
    place.task_sampler_class_for = sampler_for
    place.load_manifest = manifest_all
    place.retry_seed = lambda row,index:{k:row['sampling_seeds'][index][k] for k in ('seed_u32','seed_u64')}
