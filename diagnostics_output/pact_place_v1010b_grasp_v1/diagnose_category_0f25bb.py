...
robot_xml=Path('/root/.cache/molmo-spaces-resources/robots/franka_droid/20260127/robotiq_2f85_v4/2f85.xml')
xml=ET.parse(robot_xml);body_names={'robot_0/gripper/'+n.attrib['name'] for n in xml.findall('.//body') if 'name' in n.attrib}
...
with h5py.File(path/'grasp_diagnostics.h5') as h:
 ...
 def is_gripper(geom):
  body=int(meta['geom_body_ids'][geom]);return meta['body_all_names'][body] in body_names
 rows=[]
...
report={'schema':SCHEMA,'utc':now(),'analysis_source_sha256':sha(__file__),'sensor_source_sha256':sha(ROOT/'submodules/molmospaces/molmo_spaces/env/sensors.py'),'gripper_asset_sha256':sha(robot_xml),'gripper_body_names':sorted(body_names),
