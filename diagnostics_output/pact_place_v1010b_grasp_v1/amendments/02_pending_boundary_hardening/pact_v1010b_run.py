...
def replay_compare(rows,complete=True):
    ...
    eligible=all(r['comparison_eligible'] for r in comparisons)
    doc={'schema':SCHEMA,'count':len(rows),'comparisons':comparisons,'comparison_eligible':eligible,
        'no_demonstrated_defect':eligible,'resolution':'Exact trace or explicitly checked raster-onset diagnosis; originals remain separate. No full-trajectory identity or aggregation-causality claim.' if eligible else 'Action/outcome mismatch requires separate diagnosis; intervention blocked.'}
...
def verify():
    ...
    for stage in ('A1','A2','B','C','D'):
        ...
        for scene,group in groups.items():
            ...
            checked=len(group)>=2
            if checked:audit_initial_group(group,B/f'final_pairings/partial_{stage}_{scene}.json')
