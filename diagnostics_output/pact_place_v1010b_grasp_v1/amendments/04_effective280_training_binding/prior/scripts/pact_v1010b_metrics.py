...
def evaluate_gate(stage):
    ...
    if stage=='A':
        ...
        checks['replays_explained']=replay['all_differences_resolved']
        eligible_replays={(r['scene_id'],r['arm']) for r in replay['comparisons'] if r['comparison_eligible']}
        ...
        checks['measurable_chronology4']=sum(r['instrumented_chronology'] and (r['scene_id'],r['arm']) in eligible_replays for r in instrumented)>=4
        supported=[r for r in instrumented if r['arm']=='PACT' and not r['task_success'] and r['acquisition_discrepancy_supported'] and (r['scene_id'],r['arm']) in eligible_replays]
        checks['two_failed_acquisitions']=len(supported)>=2
        checks['no_demonstrated_defect']=replay.get('no_demonstrated_defect',False)
        details={'failure_stages':dict(stages),'restricted_replay_scopes':[r['scope_resolution'] for r in replay['comparisons'] if r.get('scope_resolution')], 'supported_ids':[r['scene_id'] for r in supported],
