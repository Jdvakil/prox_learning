"""Seed3103 joint fine-tuning with original policy math and main's encoder path."""
...
def resource_sample():
    ...
    memory = pynvml.nvmlDeviceGetMemoryInfo(device, version=2)
...
def train(stop_updates):
    ...
    if resume.exists():
        ...
        freeze(directory/'initialization_receipt.json', {'utc':now(), 'encoder_source':initial,
            'policy_seed':3103, 'policy_initialization':'fresh after setting seed3103',
        ...
        freeze(directory/'run_manifest.json', {'schema':SCHEMA, 'seed':3103,
    ...
    for epoch in range(start_epoch, 2000):
        seed_epoch(train_loader, val_loader, 3103, epoch)
        policy.eval()
        encoder.eval()
