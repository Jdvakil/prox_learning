...
import sys,argparse,collections,hashlib
...
def reconstruct(arm,scene_id,training_seed,new_directory):
 scene=next(r for r in read(C/'evaluation_manifest.json')['scenes'] if r['scene_id']==scene_id)
 paths={'original':Path(scene['frozen_directory']),'new':Path(new_directory)}
 ...
 return {'schema':SCHEMA,'analysis_code_sha256':sha(__file__),'arm':arm,'scene_id':scene_id,'training_seed':training_seed,
  'checkpoint_sha256':sha(model_dir/'policy_update_60000.ckpt'),'source_hashes':{str(path/n):sha(path/n) for path in paths.values() for n in ('initial_observation.h5','actions.npz')},'inputs':inputs,'preprocessed_rgb_changed_channels':int(np.count_nonzero(delta)),
...
if __name__=='__main__':
 ...
 freeze(C/'root_review/frozen_query_reconstruction.json',result);print(json.dumps(result,indent=2))
