...
def main():
    ...
    assert '2 passed' in (recovery / 'tests.log').read_text()
    operational_hashes = {str(recovery / name): run.sha(recovery / name) for name in ('resume.py', 'capacity_loader.py', 'train_with_capacity.py', 'test_capacity_loader.py', 'tests.log')}
    ...
    run.freeze(recovery / 'recovery.json', {
        ...
        'resume_sha256': expected['resume_sha256'], 'resume_step': 20700,
        ...
        'operational_wrapper_hashes': operational_hashes, 'wrapper_effect': 'Wait for host process capacity; all35 real batches and parent RNG verified unchanged'})
    run.atomic(run.C / 'run_status.json', {'utc': run.now(), 'status': 'RUNNING',
        'active_parent': True, 'parent_pid': run.os.getpid(), 'recovery': '04_capacity_wait'})
    ...
    try:
        ...
        job['id'] = 'training_to_60000_retry02'
        job['command'] = [run.sys.executable, str(recovery / 'train_with_capacity.py')]
        run.run_jobs([job], 1, 'training_recovery02')
